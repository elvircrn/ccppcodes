...
int main ()
{
    ...
    for (int i = 0; i < convex_hull.size() - 1; i++)
        for (int j = i + 1; j < convex_hull.size(); j++)
        {
            if (convex_hull [i].color == convex_hull [j].color)
                continue;
            else if (solution < convex_hull [i].get_distance (convex_hull [j]))
                solution = convex_hull [i].get_distance (convex_hull [j]);
            nasao = true;
        }
    ...
    if (!nasao)
    {
        for (int i = 0; i < N; i++)
            for (int j = N; j < N + M; j++)
                if (solution < points [i].get_distance (points [j]))
                    solution = points [i].get_distance (points [j]);
    }
