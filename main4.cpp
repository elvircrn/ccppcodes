...
const float cancer_patient_count  = 188.0f;
const float control_patient_count =  22.0f;

const int cancer_patient  = 188;
...
int main()
{
    ...
    out = fopen ("C:\\Users\\Inspiron\\Desktop\\codes\\vracanje u formu lol\\Prof Senol\\out667.txt", "w");
    ...
    for (int i = 0; i < common_counter; i++)
    {
        ...
        {
            if (patient_values [j] [i] == 0)
                fprintf (out, "0;");
            else
                fprintf (out, "%lf;", log2((double)patient_values [j] [i]));
        }
        ...
        for (int j = cancer_patient; j < cancer_patient + control_patient; j++)
        {
            if (patient_values [j] [i] == 0)
                ...
                fprintf (out, "%.2lf;", log2((double)patient_values [j] [i]));
        }
    }
