...
int main()
{
    ...
    while (pdir = readdir(dir))
    {
        ...
        while (fscanf (in, "%s %d %f %s", &dummy, &dummy_val, &value, &dummy) == 4)
        {
            patient_values [patient_ind] [ind] = value;
            ind++;
        }
    ...
    {
        fprintf (out2, "%s\n", code_names [i]);
        if (strncmp (code_names [i], "hsa-mir-125", strlen ("hsa-mir-125")));
            cout<<"kurac"<<endl;
    }*/
    ...
    for (int i = 0; i < mirna_count; i++)
    {
        ...
        for (int j = 0; j < mir_count; j++)
        {
            if (strncmp (mirs [j], code_names [i], strlen (mirs [j])) == 0)
            {
                found = true;
                break;
            }
