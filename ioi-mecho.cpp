...
int dirx [5] = {-1, 0, 1, 0};
int diry [5] = {0, 1, 0, -1};
int n, m, matrix [801] [801], dist [801] [801], maxd;
char unos [801];
queue <point> Q;
...
void flood_fill () {
    ...
    while (!Q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (X < 1 || Y < 1 || X > n || Y > n || (matrix [X] [Y] != 0 && matrix [X] [Y] != POZICIJA_MEDE) || dist [X] [Y] != -1) continue;
...
bool TRY (int canGo) {
    ...
    while (!Q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if ((help.w / m + canGo >= dist [X] [Y]) || X < 1 || Y < 1 || X > n || Y > n || visited [X] [Y] || matrix [X] [Y] == ROJ || matrix [X] [Y] == DRVO || matrix [X] [Y] == ROJ) continue;
