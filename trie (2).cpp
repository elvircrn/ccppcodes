...
public:
    ...
    void add(char *word, int val)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            current_node->values.push_back (val);
            if (current_node->next [convert (word [i])] == NULL)
            {
                current_node->next [convert (word [i])] = new node();
                current_node = current_node->next [convert (word [i])];
            }
            else
                current_node = current_node->next [convert (word [i])];
        }
    ...
    void parse(int contact_index)
    {
        ...
        for (;; i++)
        {
            ...
            else
            {
                ...
                if (current_node->next [convert (line [i])]->values.empty())
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        /*int asd = strlen (contacts [i]);
        for (int j = 0; j < asd - 1; j++)
        {
            if (contacts [i] [j] == ' ')
                putchar (' ');
            else
                printf ("%d", convert (contacts [i] [j]));
        } putchar ('\n');*/
        _trie.parse(i);
    } //putchar ('\n');
