...
pair <pair <int, int>, int > populate(int test_case, char **field)
{
	int n, m, direction;
	point start, end;
...
int main()
{
	...
	for (int t = 0; t < TEST_CASE; t++)
	{
		...
		/*if (solution == "nije potrebno vrsiti izmjene" && t < 5)
		{
			if (rand() % 2 == 0)
			{
				int N = rand () % p.first.first + 2;
				if (field [N] [1] == '\\')
					field [N] [1] = '/';
				else
					field [N] [1] = '\\';
			...
			solution = solve (p.first.first, p.first.second, p.second, field);
		}*/
