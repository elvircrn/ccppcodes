...
long long calculate_depth(long long current_node, long long prev_node)
{
	...
	total_sum += (cardinality [current_node] % MOD);
...
long long calculate_horizontal()
{
	...
	for (long long i = 1; i < cities.size(); i++)
	{
		...
		for (; i < cities.size(); i++)
		{
			...
			{
				if (abs (cities [i - 1].x - cities [i].x) > 1)
					current_group++;
...
long long calculate_vertical()
{
	...
	for (long long i = 1; i < cities.size(); i++)
	{
		...
		for (; i < cities.size(); i++)
		{
			...
			{
				if (abs (cities [i - 1].y - cities [i].y) > 1)
					current_group++;
				mark [cities [i]] = current_group;
				cardinality [current_group]++;
			}
