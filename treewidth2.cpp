...
void analyze(int current_index)
{
    ...
    {
        maxmax = total;
        vertexmax = current_index;
    }

    //printf ("vertex: %d total: %d\n", current_index, total);
...
int main()
{
    ...
    for (int i = 0; i < vertex_set.size(); i++)
    {
        if (vertex_set [i] == vertexmax)
            continue;

        graph [vertex_set [vertexmax]].push_back (edge (vertex_set [i], l));
        graph [vertex_set [i]].push_back (edge (vertex_set [vertexmax], l));
    }
